...
$pdf->writeHTMLCell('', '', 25, '', '<span style="font-size:16px">' . nl2br($note['CAN_IMPROVE']->load()) . '</span>', $border, 1, 0, 1, 'L');
$pdf->ln(5);
$pdf->writeHTMLCell(90, '', 20, '', '<span style="font-size:16px">2. ข้อบกพร่อง</span>', $border, 1, 0, 1, 'L');
$pdf->writeHTMLCell('', '', 25, '', '<span style="font-size:16px">' . nl2br($note['FAILING']->load()) . '</span>', $border, 1, 0, 1, 'L');
$pdf->ln(5);
$pdf->writeHTMLCell(90, '', 20, '', '<span style="font-size:16px">3. ข้อบกพร่องสำคัญมาก</span>', $border, 1, 0, 1, 'L');
$pdf->writeHTMLCell('', '', 25, '', '<span style="font-size:16px">' . nl2br($note['IMPORTANT_FAILING']->load()) . '</span>', $border, 1, 0, 1, 'L');
$pdf->ln(5);
$pdf->writeHTMLCell(90, '', 20, '', '<span style="font-size:16px">4. ข้อสังเกตจากการตรวจและข้อแนะนำ</span>', $border, 1, 0, 1, 'L');
$pdf->writeHTMLCell('', '', 25, '', '<span style="font-size:16px">' . nl2br($note['COMMENTIONS']->load()) . '</span>', $border, 1, 0, 1, 'L');
$pdf->ln(5);
